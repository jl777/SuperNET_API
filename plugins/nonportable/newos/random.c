...
int32_t OS_getppid()
{
    ...
    //PostMessage("OS_getppid.%d\n",ppid);
    //return(ppid);
}

int32_t OS_waitpid(int32_t childpid,int32_t *statusp,int32_t flags)
{
    PostMessage("OS_waitpid.%d\n",childpid);
    return(waitpid(childpid,statusp,flags));
}
...
uint32_t OS_conv_datenum(int32_t datenum,int32_t hour,int32_t minute,int32_t second) // datenum+H:M:S -> unix time
{
    ...
    PostMessage("need to find timegm()\n");
    return(0);
}
...
int32_t OS_conv_unixtime(int32_t *secondsp,time_t timestamp) // gmtime -> datenum + number of seconds
{
    ...
    {
        printf("error: timestamp.%lu -> (%d + %d) -> %u\n",(long)timestamp,datenum,*secondsp,checktime);
        return(-1);
    }
    return(datenum);
}
...
{
    pid_t child_pid;
    PostMessage("OS_launch_process\n");
    if ( (child_pid= fork()) >= 0 )
    {
        if ( child_pid == 0 )
        {
            PostMessage("plugin PID =  %d, parent pid = %d (%s, %s, %s, %s, %s)\n",getpid(),getppid(),args[0],args[1],args[2],args[3],args[4]);
            return(execv(args[0],args));
        }
        else
        {
            PostMessage("parent PID =  %d, child pid = %d\n",getpid(),child_pid);
            return(child_pid);
        }
    }
    else return(-1);
