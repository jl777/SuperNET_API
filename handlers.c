...
{
    int32_t pp_bind(char *hostname,uint16_t port);
    int32_t sock,newsock; socklen_t clilen; struct sockaddr_in cli_addr; char buffer[512];
    sock = pp_bind("127.0.0.1",8000);
    printf("bind sock.%d\n",sock);
    while ( 1 )
    {
        listen(sock,5);
        ...
        int result = (int32_t)recv(newsock, buffer, sizeof(buffer), 0);
        printf("bind recv.%d (%s)\n",result,buffer);
        close(newsock);
    }
    return(0);
}
