...
static uint32_t _crc32(uint32_t crc,const void *buf,size_t size)
{
	const uint8_t *p;
...
{
    cJSON *json;
    if ( argc > 1 && (json= cJSON_Parse(argv[1])) != 0 )
    {
        OS_init();
        process_json(json,0,1);
        free_json(json);
    }
    else printf("argc.%d %s: (%s)\n",argc,argv[0],argv[1] != 0 ? argv[1] : "");
    return(0);
}
